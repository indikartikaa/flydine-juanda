...
<div x-data="{ 
    ...
    <!-- Modul Analitik Komplain (Sistem Enterprise) -->
    @php
        $totalCats = $complaintCategories->sum('total');
        $avgMins = $avgResolutionTime ? round($avgResolutionTime) : 0;
        
        // Pemetaan Warna & Ikon Kategori
        $catStyle = function($cat) {
            if ($cat === 'pesanan_salah') return ['color' => 'rose', 'label' => 'Pesanan Salah', 'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z'];
            if ($cat === 'status_tidak_update') return ['color' => 'amber', 'label' => 'Status Lambat', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'];
            return ['color' => 'slate', 'label' => 'Lainnya', 'icon' => 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z'];
        };
    @endphp

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        
        <!-- Kartu 1: Rata-rata Waktu Penyelesaian -->
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden flex flex-col justify-center">
            <div class="absolute -right-6 -top-6 w-24 h-24 rounded-full {{ $avgMins > 60 ? 'bg-rose-50' : 'bg-emerald-50' }}"></div>
            <div class="relative z-10">
                <div class="flex items-center space-x-2 mb-2">
                    <p class="text-xs font-bold {{ $avgMins > 60 ? 'text-rose-600' : 'text-emerald-600' }} uppercase tracking-wider">Avg Resolution Time</p>
                    @if($avgMins > 60)
                        <span class="px-2 py-0.5 bg-rose-500 text-white text-[9px] font-extrabold rounded-full animate-pulse">LAMBAT</span>
                    @endif
                </div>
                <div class="flex items-baseline space-x-2">
                    <p class="text-4xl font-extrabold text-slate-800">{{ $avgMins }}</p>
                    <p class="text-sm font-semibold text-slate-500">Menit</p>
                </div>
                <p class="text-[11px] text-slate-400 mt-2 font-medium">
                    Waktu rata-rata tim menyelesaikan keluhan sejak dibuat.
                </p>
            </div>
        </div>

        <!-- Kartu 2: Distribusi Kategori Akar Masalah -->
        <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-center">
            <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider mb-4">Akar Masalah Utama (Root Cause)</h3>
            
            @if($totalCats > 0)
                <div class="space-y-4">
                    @foreach($complaintCategories as $category)
                        @php
                            $style = $catStyle($category->category);
                            $pct = round(($category->total / $totalCats) * 100);
                        @endphp
                        <div>
                            <div class="flex items-center justify-between text-[11px] font-bold mb-1">
                                <span class="text-slate-700 flex items-center">
                                    <svg class="w-3.5 h-3.5 mr-1.5 text-{{ $style['color'] }}-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $style['icon'] }}"></path></svg>
                                    {{ $style['label'] }}
                                </span>
                                <span class="text-{{ $style['color'] }}-600">{{ $category->total }} Kasus ({{ $pct }}%)</span>
                            </div>
                            <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                                <div class="bg-{{ $style['color'] }}-500 h-2 rounded-full transition-all duration-1000" style="width: {{ $pct }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
            @else
                <div class="text-center py-4 text-slate-400 text-xs font-medium border-2 border-dashed border-slate-100 rounded-xl">
                    Belum ada data komplain untuk dianalisis.
                </div>
            @endif
        </div>
    </div>
